...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const ctx = gsap.context(() => {
        ...
        const socialIcons = heroRef.current?.querySelectorAll('.social-icon');
  ...
  useEffect(() => {
    ...
    if (!cursor || !cursorDot || !heroSection) return;
    ...
    const handleMouseLeave = () => {
      ...
      gsap.to(cursorDot, { opacity: 0, scale: 0, duration: 0.3 });
    };
  ...
  return (
    ...
    >
      {/* Custom Cursor Effect */}
      <div className="movingCursor absolute top-0 left-0 w-12 h-12 pointer-events-none z-50 opacity-0 scale-0">
        <div className="absolute inset-0 border-2 border-[var(--primary)] rounded-full animate-pulse"></div>
        <div className="absolute inset-2 bg-[var(--primary)] bg-opacity-20 rounded-full backdrop-blur-sm"></div>
      </div>
      <div className="cursor-dot absolute top-0 left-0 w-2 h-2 pointer-events-none z-50 opacity-0 scale-0 -translate-x-1/2 -translate-y-1/2">
        <div className="absolute inset-0 bg-[var(--secondary)] rounded-full shadow-lg shadow-[var(--secondary)]"></div>
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div ref={socialRef} className="flex flex-wrap justify-center gap-4 mb-16">
          {[
            ...
            { icon: FaFacebook, href: 'https://www.facebook.com/aneeesPGP/', color: '#1877F2', label: 'Facebook' },
